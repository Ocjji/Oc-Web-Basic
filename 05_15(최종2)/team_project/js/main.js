...
const pack = () => {
    ...
    const $packV_btn_o_prev = get(
        ".packVis .packV_spop_people .packV_btn_o_prev"
    );
    const $packV_spop_btn_ol = get(".packVis .packV_o .ol");
    const $packV_btn_o_next = get(
        ".packVis .packV_spop_people .packV_btn_o_next"
    );
    const $packV_btn_y_prev = get(
        ".packVis .packV_spop_people .packV_btn_y_prev"
    );
    const $packV_spop_btn_yo = get(".packVis .packV_y .yo");
    const $packV_btn_y_next = get(
        ".packVis .packV_spop_people .packV_btn_y_next"
    );
    //searh_btt
    ...
    let packV_slid_num = 0;
    let packV_down, packV_up;
    let packV_slid_people = true;
    let packV_slid_date = true;
    let packV_slid_go = true;
    let pack_nowMonth = new Date(); // 현재 달을 페이지를 로드한 날의 달로 초기화
    let pack_today = new Date(); // 페이지를 로드한 날짜를 저장
    pack_today.setHours(0, 0, 0, 0); // 비교 편의를 위해 pack_today의 시간을 초기화
    window.onload = function () {
        pack_buildCal();
    }; // 웹 페이지가 로드되면 pack_buildCal 실행
    city_jeju();
    city_seo();
    ...
    function pack_buildCal() {
        ...
        document.getElementById("calYear").innerText = pack_nowMonth.getFullYear(); // 연도 숫자 갱신
        document.getElementById("calMonth").innerText = pack_leftPad(
            pack_nowMonth.getMonth() + 1
        ); // 월 숫자 갱신
        ...
        for (
            ...
            newDIV.innerHTML = pack_leftPad(nowDay.getDate()); // 추가한 열에 날짜 입력
    ...
    function pack_choiceDate(newDIV) {
        if (document.getElementsByClassName("pack_choiceDay")[0]) {
            // 기존에 선택한 날짜가 있으면
            document
                .getElementsByClassName("pack_choiceDay")[0]
                .classList.remove("pack_choiceDay"); // 해당 날짜의 "pack_choiceDay" class 제거
        }
        newDIV.classList.add("pack_choiceDay"); // 선택된 날짜에 "pack_choiceDay" class 추가

        $packV_search_go_txt.textContent = `${pack_nowMonth.getMonth() + 1}월 ${newDIV.textContent
            }일`;
        $packV_search_go_txt.style.color = "black";
    }
    // 이전달 버튼 클릭
    function pack_prevCal() {
        pack_nowMonth = new Date(
            pack_nowMonth.getFullYear(),
            pack_nowMonth.getMonth() - 1,
            pack_nowMonth.getDate()
        ); // 현재 달을 1 감소
        pack_buildCal(); // 달력 다시 생성
    }
    // 다음달 버튼 클릭
    function pack_nextCal() {
        pack_nowMonth = new Date(
            pack_nowMonth.getFullYear(),
            pack_nowMonth.getMonth() + 1,
            pack_nowMonth.getDate()
...
function notice() {
    ...
    searchSelect.addEventListener('change', e => {
        ...
        console.log(seletedCurrent.value);
    ...
    noticeBoardLi.forEach((element, idx) => {
        element.addEventListener('click', e => {
            ...
            console.log(e.currentTarget.dataset.categorynum);
